...
namespace NeonClusterOperator
{
    ...
    public class NeonClusterJobController : ResourceControllerBase<V1NeonClusterJobConfig>
    {
        ...
        private const string JobGroup = nameof(NeonClusterJobController);
        ...
        public override async Task<ResourceControllerResult> ReconcileAsync(V1NeonClusterJobConfig resource, CancellationToken cancellationToken = default)
        {
            ...
            using (var activity = TelemetryHub.ActivitySource?.StartActivity())
            {
                ...
                // schedule from the [V1NeonClusterJobConfig] resource.

                await ScheduleJobAsync<MinWorkerNodeVcpuJob>(
                    scheduler,
                    k8s,
                    "0 30 1 ? * *");    // 1:30am everyday
        ...
        /// </summary>
        /// <param name="cronSchedule">Specifies the CRON schedule from <see cref="V1NeonClusterJobConfig"/>.</param>
        /// <param name="jobStatus">Specifies the job status for the target job from <see cref="V1NeonClusterJobConfig"/>  (or <c>null</c>).</param>
        /// <returns>The resolved CRON schedule for the job.</returns>
        public string GetResolvedJobSchedule(string cronSchedule, V1NeonClusterJobConfig.JobStatus jobStatus)
        {
