...
namespace TestHive
{
    public partial class Test_HiveLoadBalancer : IClassFixture<HiveFixture>
    {
        ...
        /// <summary>
        /// Generates a load balancer rule for each <see cref="Redirection"/> passed that
        /// will redirect from one URI to another.
        /// </summary>
        /// <param name="loadBalancer">The target load balancer.</param>
        /// <param name="testName">Used to name the load balancer rules.</param>
        ...
        private async Task TestRedirect(LoadBalancerManager loadBalancer, string testName, bool singleRule, params Redirection[] redirections)
        {
            ...
            // Create the load balancer rule(s).
        ...
        {
            await TestRedirect(hive.PublicLoadBalancer, "test-a", singleRule: true, redirections:
                new Redirection[]
                {
        ...
        {
            await TestRedirect(hive.PrivateLoadBalancer, "test-a", singleRule: true, redirections:
                new Redirection[]
                {
