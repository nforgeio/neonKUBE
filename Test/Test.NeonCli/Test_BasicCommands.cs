    ...
    public class Test_BasicCommands
    {
        ...
        public async Task Version()
        {
            using (var runner = new ProgramRunner())
            {
                ...
                Assert.Equal(0, result.ExitCode);
                Assert.Equal(KubeVersion.Kubernetes, result.OutputText.Trim());
                ...
                Assert.Equal(0, result.ExitCode);
                Assert.Equal($"{KubeVersion.Kubernetes}/{ThisAssembly.Git.Branch}-{ThisAssembly.Git.Commit}", result.OutputText.Trim());
                Assert.DoesNotContain('\n', result.OutputText);
