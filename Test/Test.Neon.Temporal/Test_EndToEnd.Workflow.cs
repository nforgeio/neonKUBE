...
namespace TestTemporal
{
    public partial class Test_EndToEnd
    {
        ...
        public void Workflow_GetWorkflowTypeName()
        {
            ...
            const string baseTypeName = "TestTemporal.Test_EndToEnd.WorkflowWithResult";

            Assert.Equal(baseTypeName, TemporalHelper.GetWorkflowTypeName<IWorkflowWithResult>());
            Assert.Equal(baseTypeName, TemporalHelper.GetWorkflowTypeName<IWorkflowWithResult>(""));
            Assert.Equal(baseTypeName + "::run", TemporalHelper.GetWorkflowTypeName<IWorkflowWithResult>("run"));
        ...
        public class WorkflowQuery : WorkflowBase, IWorkflowQuery
        {
            ...
            public async Task<List<string>> RunAsync(TimeSpan timeout, int expectedQueries)
            {
                ...
                throw new TemporalTimeoutException("Timeout waiting for query(s).");
            }
        ...
        public class WorkflowComplex : WorkflowBase, IWorkflowComplex
        {
            ...
            public async Task<List<string>> WaitForQueriesAndSignalsAsync(TimeSpan timeout, int expectedOperations)
            {
                ...
                throw new TemporalTimeoutException("Timeout waiting for queries and/or signals.");
            }
        ...
        public class WorkflowContinueAsNew0 : WorkflowBase, IWorkflowContinueAsNew0
        {
            ...
            public async Task<string> HelloStubOptionsAsync(string name)
            {
                ...
                var options  = new ContinueAsNewOptions() 
                {
                    Workflow = "TestTemporal.Test_EndToEnd.WorkflowContinueAsNew1"
                };
        ...
        public class WorkflowIdReuse : WorkflowBase, IWorkflowIdReuse
        {
            ...
            {
                // Verify that we can have Temporal allow duplicate workflow IDs
                // using child options.
        ...
        public class WorkflowQueueTest : WorkflowBase, IWorkflowQueueTest
        {
            ...
            {
                // Verifies that [temporal-proxy] honors dequeuing timeouts.
                ...
                using (var queue = await Workflow.NewQueueAsync<string>())
                {
                    ...
                    catch (TemporalTimeoutException)
                    {
        ...
        public class WorkflowTimeout : WorkflowBase, IWorkflowTimeout
        {
            ...
            public async Task<bool> ActivityHeartbeatTimeoutAsync()
            {
                ...
                // longer than its heartbeat interval.  Temporal should
            ...
            public async Task<bool> ActivityDotNetException()
            {
                ...
                // verify that we see a [TemporalGenericException] formatted
        ...
        public async Task Workflow_Container()
        {
            ...
            // generate the Temporal URI we'll pass to the [test-temporal] container so it
            // will be able to connect to the Temporal server running locally.
            ...
            // Start the [test-temporal] container and give it a chance to connect to Temporal
            ...
            var testTemporalImage = $"{KubeConst.NeonBranchRegistry}/test-temporal:latest";
            ...
            if (exitCode != 0)
            {
                Assert.True(false, $"Cannot pull the [{testTemporalImage}] Docker image.");
            }
            ...
            if (exitCode != 0)
            {
                Assert.True(false, $"Cannot run the [{testTemporalImage}] Docker image.");
            }
            ...
            finally
            {
                ...
                if (exitCode != 0)
                {
                    Assert.True(false, $"Cannot remove the [{testTemporalImage}] Docker container.");
                }
            }
        }
