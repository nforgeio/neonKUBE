...
{
    /// <summary>
    /// <b>INTERNAL USE ONLY:</b> Describes a workflow execution.  This maps directly to the Temporal GOLANG <b>WorkflowExecutionInfo</b> structure. 
    /// </summary>
    internal class InternalWorkflowExecutionInfo
    {
        ...
        private TimeSpan ComputeExecutionTime()
        {
            ...
            {
                return TemporalHelper.UnixNanoToDateTimeUtc(this.StartTime) - TemporalHelper.UnixNanoToDateTimeUtc(this.CloseTime);
            }
            else if (this.StartTime > 0)
            {
                var executionTime = DateTime.UtcNow - TemporalHelper.UnixNanoToDateTimeUtc(this.StartTime);

                // It's possible for this calculation to come out negative when the Temporal server
        ...
        public WorkflowStatus ToPublic()
        {
            ...
            if (this.StartTime > 0)
            {
                executionState.StartTime = TemporalHelper.UnixNanoToDateTimeUtc(this.StartTime);
            }

            if (this.CloseTime > 0)
            {
                executionState.CloseTime = TemporalHelper.UnixNanoToDateTimeUtc(this.CloseTime);
            }
