...
namespace Neon.Temporal
{
    ...
    public class WorkflowBase
    {
        ...
        /// <summary>
        /// Prepends the Temporal client ID to the workflow type name as well as the optional
        ...
        private static string GetWorkflowTypeNameFromKey(string workflowTypeKey)
        {
            ...
            var separatorPos = workflowTypeKey.IndexOf(TemporalHelper.WorkflowTypeMethodSeparator);

            Covenant.Assert(separatorPos >= 0);

            return workflowTypeKey.Substring(separatorPos + TemporalHelper.WorkflowTypeMethodSeparator.Length);
        }
        ...
        /// <summary>
        /// Removes all type workflow interface registrations for a Temporal client (when it's being disposed).
        /// </summary>
        ...
        internal static async Task<WorkflowInvokeReply> OnInvokeAsync(TemporalClient client, WorkflowInvokeRequest request)
        {
            ...
            lock (syncLock)
            {
                if (idToWorkflow.TryGetValue(workflowKey, out workflow))
                {
                    ...
                    {
                        Error = new TemporalError($"A workflow with [ID={workflowKey}] is already running on this worker.")
                    };
                }
