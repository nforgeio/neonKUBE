...
namespace Neon.Kube.Setup
{
    public static partial class KubeSetup
    {
        ...
        public static void CalculateResourceRequirements(ISetupController controller)
        {
            ...
            clusterAdvice.AddServiceAdvice(KubeClusterAdvice.NeonSystemDbOperator, CalculateNeonSystemDbOperatorAdvice(cluster));
        ...
        private static KubeServiceAdvice CalculateNeonSystemDbOperatorAdvice(ClusterProxy cluster)
        {
            var advice = new KubeServiceAdvice(KubeClusterAdvice.NeonSystemDbOperator);
        ...
        private static KubeServiceAdvice CalculateEtcdClusterAdvice(ClusterProxy cluster)
        {
            ...
            advice.ReplicaCount = Math.Min(3, (cluster.SetupState.ClusterDefinition.Nodes.Where(node => node.Labels.SystemMetricServices).Count()));
        ...
        private static KubeServiceAdvice CalculateMinioAdvice(ClusterProxy cluster)
        {
            ...
            if (cluster.SetupState.ClusterDefinition.Nodes.Where(node => node.Labels.SystemMinioServices).Count() >= 3)
            {
                if (cluster.SetupState.ClusterDefinition.Nodes.Where(node => node.Labels.SystemMinioServices).Count() >= 4)
                {
                    advice.ReplicaCount = cluster.SetupState.ClusterDefinition.Nodes.Where(node => node.Labels.SystemMinioServices).Count();
                }
                else
                {
                    advice.ReplicaCount = 1;
        ...
        private static KubeServiceAdvice CalculateOpenEbsCstorPoolAdvice(ClusterProxy cluster)
        {
            ...
            advice.ReplicaCount = Math.Min(3, (cluster.SetupState.ClusterDefinition.Nodes.Where(n => n.Labels.SystemMetricServices).Count()));
