...
{
    /// <summary>
    /// The base class for traffic director rules.
    /// </summary>
    public class TrafficDirectorRule
    {
        //---------------------------------------------------------------------
        // Static members
        ...
        /// <summary>
        /// Parses a <see cref="TrafficDirectorRule"/> from a JSON or YAML string,
        ...
        /// <returns>The parsed object instance derived from <see cref="TrafficDirectorRule"/>.</returns>
        public static TrafficDirectorRule Parse(string jsonOrYaml, bool strict = false)
        {
        ...
        /// <summary>
        /// Parses a <see cref="TrafficDirectorRule"/> from a JSON string.
        /// </summary>
        /// <param name="jsonText">The input string.</param>
        /// <param name="strict">Optionally require that all input properties map to rule properties.</param>
        /// <returns>The parsed object instance derived from <see cref="TrafficDirectorRule"/>.</returns>
        public static TrafficDirectorRule ParseJson(string jsonText, bool strict = false)
        {
            ...
            var baseRule = NeonHelper.JsonDeserialize<TrafficDirectorRule>(jsonText);

            // Now that we know the rule mode, we can deserialize the full rule.

            switch (baseRule.Mode)
            {
                case TrafficDirectorMode.Http:

                    return NeonHelper.JsonDeserialize<TrafficDirectorHttpRule>(jsonText, strict);

                case TrafficDirectorMode.Tcp:

                    return NeonHelper.JsonDeserialize<TrafficDirectorTcpRule>(jsonText, strict);

                default:

                    throw new NotImplementedException($"Unsupported [{nameof(TrafficDirectorRule)}.{nameof(Mode)}={baseRule.Mode}].");
            }
        }

        /// <summary>
        /// Parses a <see cref="TrafficDirectorRule"/> from a YAML string.
        /// </summary>
        /// <param name="yamlText">The input string.</param>
        /// <param name="strict">Optionally require that all input properties map to rule properties.</param>
        /// <returns>The parsed object instance derived from <see cref="TrafficDirectorRule"/>.</returns>
        public static TrafficDirectorRule ParseYaml(string yamlText, bool strict = false)
        {
            ...
            var baseRule = NeonHelper.YamlDeserialize<TrafficDirectorRule>(yamlText, strict: false);
            ...
            switch (baseRule.Mode)
            {
                case TrafficDirectorMode.Http:
                    ...
                    return NeonHelper.YamlDeserialize<TrafficDirectorHttpRule>(yamlText, strict);

                case TrafficDirectorMode.Tcp:

                    return NeonHelper.YamlDeserialize<TrafficDirectorTcpRule>(yamlText, strict);
        ...
        /// <summary>
        /// Indicates whether HTTP or TCP traffic is to be handled (defaults to <see cref="TrafficDirectorMode.Http"/>).
        /// </summary>
        [JsonProperty(PropertyName = "Mode", Required = Required.Always)]
        [DefaultValue(TrafficDirectorMode.Http)]
        public TrafficDirectorMode Mode { get; set; } = TrafficDirectorMode.Http;
        ...
        /// We defaulted this to <b>docker</b> because we expect the most traffic director rules
        ...
        /// <see cref="TrafficDirectorCheckMode.Default"/> which will perform TCP connection 
        /// checks for <see cref="TrafficDirectorTcpRule"/> rules and HTTP checks for 
        /// <see cref="TrafficDirectorHttpRule"/> rules.
        /// </summary>
        /// <remarks>
        /// <para>
        /// You may set this to <see cref="TrafficDirectorCheckMode.Http"/> to perform HTTP
        /// checks for <see cref="TrafficDirectorTcpRule"/> rules or <see cref="TrafficDirectorCheckMode.Tcp"/>
        /// to perform TCP checks for <see cref="TrafficDirectorHttpRule"/> rules.
        /// </para>
        /// <para>
        /// You can also set this to <see cref="TrafficDirectorCheckMode.Disabled"/> to disable
        ...
        [DefaultValue(TrafficDirectorCheckMode.Default)]
        public TrafficDirectorCheckMode CheckMode { get; set; } = TrafficDirectorCheckMode.Default;
        ...
        /// This can be enabled  for both <see cref="TrafficDirectorHttpRule"/> and 
        /// <see cref="TrafficDirectorTcpRule"/> rules.  This defaults to <c>false</c>.
        /// </summary>
        ...
        /// <summary>
        /// <para>
        /// The relative URI the traffic director will use to verify the backend server health when <see cref="TrafficDirectorRule.CheckMode"/> is <c>true</c> .  
        ...
        /// </para>
        /// <para>
        /// You can also set this to <c>null</c> to enable simple TCP connect checks if <see cref="TrafficDirectorRule.CheckMode"/> 
        /// is enabled.
        /// </para>
        /// </summary>
        ...
        [DefaultValue(null)]
        public List<TrafficDirectorCheckHeader> CheckHeaders { get; set; } = new List<TrafficDirectorCheckHeader>();
        ...
        /// <summary>
        /// The endpoint timeouts.  These default to standard timeouts defined by <see cref="TrafficDirectorTimeouts"/>.
        /// </summary>
        [JsonProperty(PropertyName = "Timeouts", Required = Required.Default, DefaultValueHandling = DefaultValueHandling.IgnoreAndPopulate)]
        [DefaultValue(null)]
        public TrafficDirectorTimeouts Timeouts { get; set; } = null;
        ...
        public bool UseHttpCheckMode
        {
            get
            {
                switch (CheckMode)
                {
                    case TrafficDirectorCheckMode.Default:  return this is TrafficDirectorHttpRule;
                    case TrafficDirectorCheckMode.Disabled: return false;
                    case TrafficDirectorCheckMode.Http:     return true;
                    case TrafficDirectorCheckMode.Tcp:      return false;
                    default:                                throw new NotImplementedException($"Unexpected traffic director [{nameof(CheckMode)}={CheckMode}].");
                }
            }
        }
        ...
        public virtual void Validate(TrafficDirectorValidationContext context)
        {
            Timeouts = Timeouts ?? new TrafficDirectorTimeouts();
            ...
            CheckHeaders = CheckHeaders ?? new List<TrafficDirectorCheckHeader>();
