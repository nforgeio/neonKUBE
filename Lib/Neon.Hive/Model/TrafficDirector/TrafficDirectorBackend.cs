//-----------------------------------------------------------------------------
// FILE:	    TrafficDirectorBackend.cs
...
{
    /// <summary>
    /// Base class for traffic director backends.
    /// </summary>
    public class TrafficDirectorBackend
    {
        /// <summary>
        ...
        public virtual void Validate(TrafficDirectorValidationContext context, string ruleName)
        {
        ...
        public List<NodeDefinition> SelectGroupNodes(Dictionary<string, List<NodeDefinition>> hostGroups)
        {
            ...
            if (string.IsNullOrEmpty(Group))
            {
                throw new InvalidOperationException($"[{nameof(TrafficDirectorBackend)}.{nameof(Group)}()] works only for rule backends that target a group.");
            }
