//-----------------------------------------------------------------------------
// FILE:	    TrafficDirectorHttpFrontend.cs
...
{
    /// <summary>
    /// Describes an HTTP/HTTPS traffic director frontend.
    /// </summary>
    public class TrafficDirectorHttpFrontend
    {
        /// <summary>
        /// <para>
        ...
        /// </para>
        /// <note>
        /// This is required for rules targeting the traffic director's default HTTP/S 
        ...
        /// </para>
        /// <note>
        /// The traffic director will match longer prefixes before shorter ones.
        /// </note>
        /// </remarks>
        ...
        /// <summary>
        /// The optional HAProxy frontend port number.  This defaults to the traffic director's default HTTPS port when a certificate name
        ...
        /// </para>
        /// <note>
        /// This is honored only for <b>public</b> traffic director rules.  Public ports for <b>private</b> proxies will be ignored.
        /// </note>
        /// </summary>
        /// <remarks>
        /// <para>
        ...
        public void Validate(TrafficDirectorValidationContext context, TrafficDirectorHttpRule rule)
        {
                ...
                ProxyPort == HiveHostPorts.ProxyPrivateHttp || ProxyPort == HiveHostPorts.ProxyPrivateHttps)
            {
                ...
                if (string.IsNullOrEmpty(Host))
                {
                    context.Error($"Rule [{rule.Name}] has a frontend without a [{nameof(Host)}] specified.  HTTP rules targeting the default traffic director HTTP/S ports, with more than one frontend, or secured by TLS requires frontend hostnames.");
                }
            ...
            if (ProxyPort != 0)
            {
                ...
                {
                    context.Error($"Rule [{rule.Name}] assigns [{nameof(ProxyPort)}={ProxyPort}] which is outside the range of valid frontend ports for this traffic director [{context.Settings.ProxyPorts}].");
                }
            }
            else
            {
