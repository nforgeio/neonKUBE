//-----------------------------------------------------------------------------
// FILE:	    TrafficDirectorSettings.cs
...
{
    /// <summary>
    /// Describes the global settings for a neonHIVE traffic director.
    /// </summary>
    public class TrafficDirectorSettings
    {
        //---------------------------------------------------------------------
        // Static members
        ...
        /// <summary>
        /// Parses a <see cref="TrafficDirectorSettings"/> from a JSON or YAML string,
        ...
        /// <returns>The parsed <see cref="TrafficDirectorSettings"/>.</returns>
        public static TrafficDirectorSettings Parse(string jsonOrYaml, bool strict = false)
        {
        ...
        /// <param name="strict">Optionally require that all input properties map to settings properties.</param>
        /// <returns>The parsed <see cref="TrafficDirectorSettings"/>.</returns>
        public static TrafficDirectorSettings ParseJson(string jsonText, bool strict = false)
        {
            Covenant.Requires<ArgumentNullException>(!string.IsNullOrEmpty(jsonText));

            return NeonHelper.JsonDeserialize<TrafficDirectorSettings>(jsonText, strict);
        }
        ...
        /// <param name="strict">Optionally require that all input properties map to settings properties.</param>
        /// <returns>The parsed <see cref="TrafficDirectorSettings"/>.</returns>
        public static TrafficDirectorSettings ParseYaml(string yamlText, bool strict = false)
        {
            Covenant.Requires<ArgumentNullException>(!string.IsNullOrEmpty(yamlText));

            return NeonHelper.YamlDeserialize<TrafficDirectorSettings>(yamlText, strict);
        }
        ...
        public int FirstTcpPort
        {
            get
            {
                ...
                {
                    Covenant.Assert(false, "Unexpected traffic director configuration.");
        ...
        /// to be allowed for the traffic director.  (Defaults to <b>32000</b>).
        /// </summary>
        ...
        [DefaultValue(null)]
        public TrafficDirectorTimeouts Timeouts { get; set; } = new TrafficDirectorTimeouts();

        /// <summary>
        /// <para>
        /// The DNS resolvers available for use by the traffic director's rules.
        /// </para>
        /// <note>
        ...
        public List<TrafficDirectorResolver> Resolvers { get; set; } = new List<TrafficDirectorResolver>();
        ...
        /// traffic director targets.  This can be overridden by explicity designating 
        ...
        /// be designated as bridge traffic director targets.  If there are more workers, then
        ...
        public void Validate(TrafficDirectorValidationContext context)
        {
            ...
            Resolvers             = Resolvers ?? new List<TrafficDirectorResolver>();
            BridgeTargetAddresses = BridgeTargetAddresses ?? new List<IPAddress>();

            if (!Resolvers.Exists(r => r.Name == "docker"))
            {
                Resolvers.Add(
                    new TrafficDirectorResolver()
                    {
                        Name = "docker",
                        NameServers = new List<TrafficDirectorNameserver>()
                        {
                            new TrafficDirectorNameserver()
                            {
                                Name     = "docker0",
