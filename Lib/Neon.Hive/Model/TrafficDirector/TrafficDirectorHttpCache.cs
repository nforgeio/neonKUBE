//-----------------------------------------------------------------------------
// FILE:	    TrafficDirectorHttpCache.cs
...
namespace Neon.Hive
{
    ...
    /// </summary>
    public class TrafficDirectorHttpCache
    {
        private const int defaultWarmSeconds = 120;

        /// <summary>
        /// Enables HTTP caching for a <see cref="TrafficDirectorHttpRule"/>.  This defaults to <c>false</c>.
        /// </summary>
        ...
        /// for the associated traffic director rule when caching is enabled.  Details will be returned as one
        ...
        [DefaultValue(null)]
        public List<TrafficDirectorWarmTarget> WarmTargets { get; set; } = new List<TrafficDirectorWarmTarget>();
        ...
        public void Validate(TrafficDirectorValidationContext context)
        {
            if (DnsTTL < 1)
            {
                context.Error($"[{nameof(TrafficDirectorHttpCache)}.{nameof(DnsTTL)}={DnsTTL}] cannot be less than 1 second.");
            }
