//-----------------------------------------------------------------------------
// FILE:	    TrafficDirectorHttpRule.cs
...
{
    /// <summary>
    /// Describes a traffic director rule that forwards traffic from 
    /// HTTP and/or HTTPS frontends to HTTP backend servers.
    /// </summary>
    public class TrafficDirectorHttpRule : TrafficDirectorRule
    {
        private List<TrafficDirectorHttpBackend> selectedBackends;    // Used to cache selected backends

        /// <summary>
        /// Default constructor.
        /// </summary>
        public TrafficDirectorHttpRule()
        {
            base.Mode = TrafficDirectorMode.Http;
        }

        /// <summary>
        /// The traffic director frontend definitions.
        /// </summary>
        [JsonProperty(PropertyName = "Frontends", Required = Required.Always)]
        public List<TrafficDirectorHttpFrontend> Frontends { get; set; } = new List<TrafficDirectorHttpFrontend>();

        /// <summary>
        /// The traffic director backend definitions.
        /// </summary>
        [JsonProperty(PropertyName = "Backends", Required = Required.Always)]
        public List<TrafficDirectorHttpBackend> Backends { get; set; } = new List<TrafficDirectorHttpBackend>();
        ...
        [DefaultValue(null)]
        public TrafficDirectorHttpCache Cache { get; set; }

        /// <summary>
        /// Returns the list of backends selected to be targeted by processing any
        /// backends with <see cref="TrafficDirectorBackend.Group"/> and <see cref="TrafficDirectorBackend.GroupLimit"/>
        ...
        /// </note>
        /// </remarks>
        public List<TrafficDirectorHttpBackend> SelectBackends(Dictionary<string, List<NodeDefinition>> hostGroups)
        {
            ...
            selectedBackends = new List<TrafficDirectorHttpBackend>();
        ...
        public override void Validate(TrafficDirectorValidationContext context)
        {
            base.Validate(context);

            Frontends = Frontends ?? new List<TrafficDirectorHttpFrontend>();
            Backends  = Backends ?? new List<TrafficDirectorHttpBackend>();
            ...
            if (Cache != null && Cache.Enabled)
            {
                ...
                foreach (var warmTarget in Cache.WarmTargets)
                {
                    ...
                    {
                        context.Error($"Cache warm target [{uri}] does not match one of the [{Name}] traffic director frontends.");
                    }
                }
            }
        }
