...
namespace Neon.Hive
{
    ...
    public sealed class DnsHostsManager
    {
        ...
        public DnsEntry Get(string hostname)
        {
            ...
            return cluster.Consul.Client.KV.GetObjectOrDefault<DnsEntry>($"{HiveConst.ConsulDnsEntriesKey}/{hostname}").Result;
        }
        ...
        public List<DnsEntry> List(bool includeSystem = false)
        {
            ...
            foreach (var key in cluster.Consul.Client.KV.ListKeys(HiveConst.ConsulDnsEntriesKey, ConsulListMode.PartialKey).Result)
            {
                var entry = cluster.Consul.Client.KV.GetObjectOrDefault<DnsEntry>($"{HiveConst.ConsulDnsEntriesKey}/{key}").Result;
        ...
        public void Remove(string hostname, bool waitUntilPropagated = false)
        {
            ...
            cluster.Consul.Client.KV.Delete($"{HiveConst.ConsulDnsEntriesKey}/{hostname}").Wait();
        ...
        public void Set(DnsEntry entry, bool waitUntilPropagated = false)
        {
            ...
            cluster.Consul.Client.KV.PutObject($"{HiveConst.ConsulDnsEntriesKey}/{hostname}", entry).Wait();
        ...
        {
            var hosts  = cluster.Consul.Client.KV.GetStringOrDefault(HiveConst.ConsulDnsHostsKey).Result;
