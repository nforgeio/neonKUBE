...
namespace Neon.Cadence
{
    ...
    public class ActivityOptions
    {
        ...
        internal static ActivityOptions Normalize(CadenceClient client, ActivityOptions options, Type activityInterface = null)
        {
            ...
            if (string.IsNullOrEmpty(options.TaskList))
            {
                throw new ArgumentNullException(nameof(options), "You must specify a valid task list explicitly in [ActivityOptions] or via an [ActivityInterface(TaskList = \"my-tasklist\")] attribute on the target activity interface.");
            }
