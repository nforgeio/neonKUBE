...
{
    /// <summary>
    /// Used to instantiate code generated classes that implement <see cref=" IRoundtripType"/>
    ...
    public static class RoundtripDataFactory 
    {
...
#if DEBUG
            Covenant.Requires<ArgumentException>(resultType.Implements<IRoundtripType>());
#endif
        ...
        public static async Task<object> CreateFromAsync(Type resultType, Stream stream, Encoding encoding = null)
        {
            ...
            lock (classNameToStreamCreateMethod)
            {
                ...
                {
                    if (!resultType.Implements<IRoundtripType>())
                    {
                        throw new InvalidOperationException($"Type [{resultType.FullName}] does not implement [{nameof(IRoundtripType)}].");
                    }
        ...
        /// <param name="encoding">Optionally specifies the encoding (defaults to UTF-8).</param>
        /// <returns>
        /// <c>true</c> if the object type implements <see cref="IRoundtripType"/> and the 
