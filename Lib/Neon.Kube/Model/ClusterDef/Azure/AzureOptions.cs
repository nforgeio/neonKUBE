...
namespace Neon.Kube
{
    ...
    public class AzureOptions
    {
        ...
        {
            get { return FirstVpnFrontendPort + ClusterConst.MaxManagers - 1; }
        }
        ...
        public void Validate(ClusterDefinition clusterDefinition)
        {
            ...
            foreach (var ch in clusterDefinition.Name)
            {
                ...
                throw new ClusterDefinitionException($"Hive name [{clusterDefinition.Name}] is not valid for Azure deployment.  Only letters, digits, dashes, or underscores are allowed.");
            }
            ...
            if (clusterDefinition.Managers.Count() > ClusterConst.MaxManagers)
            {
                throw new ClusterDefinitionException($"Hive manager count [{clusterDefinition.Managers.Count()}] exceeds the [{ClusterConst.MaxManagers}] limit for neonHIVEs.");
            }

            if (clusterDefinition.Nodes.Count() > AzureHelper.MaxHiveNodes)
            {
                throw new ClusterDefinitionException($"Hive node count [{clusterDefinition.Nodes.Count()}] exceeds the [{AzureHelper.MaxHiveNodes}] limit for neonHIVEs deployed to Azure.");
            }
        }
    }
}
