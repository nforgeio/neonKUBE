...
namespace Neon.Kube
{
    ...
    public class NetworkOptions
    {
        ...
        /// FQDN of the public cluster traffic manager responsible for forwarding traffic
        ...
        /// <summary>
        /// Optionally specifies the cluster's worker/pet public FQDN or IP address during 
        ...
        public void Validate(KubeDefinition kubeDefinition)
        {
            ...
            {
                throw new KubeDefinitionException($"Invalid [{nameof(NetworkOptions)}.{nameof(PublicSubnet)}={PublicSubnet}].");
            }
            ...
            {
                throw new KubeDefinitionException($"Invalid [{nameof(NetworkOptions)}.{nameof(PrivateSubnet)}={PrivateSubnet}].");
            }
            ...
            {
                throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(PublicSubnet)}] cannot be the same as [{nameof(PrivateSubnet)}] .");
            }
            ...
            foreach (var nameserver in Nameservers)
            {
                ...
                {
                    throw new KubeDefinitionException($"[{nameserver}] is not a valid [{nameof(NetworkOptions)}.{nameof(Nameservers)}] IP address.");
                }
            }
            ...
            if (kubeDefinition.Hosting.IsCloudProvider)
            {
                ...
                {
                    throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(CloudSubnet)}={CloudSubnet}] is not a valid IPv4 subnet.");
                }
            ...
            else
            {
                ...
                {
                    throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(PremiseSubnet)}={PremiseSubnet}] is not a valid IPv4 subnet.");
                }
                ...
                {
                    throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(Gateway)}={Gateway}] is not a valid IPv4 address.");
                }

                if (!premiseCidr.Contains(gateway))
                {
                    throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(Gateway)}={Gateway}] address is not within the [{nameof(NetworkOptions)}.{nameof(NetworkOptions.NodesSubnet)}={NodesSubnet}] subnet.");
                }
                ...
                {
                    throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(Broadcast)}={Broadcast}] is not a valid IPv4 address.");
                }

                if (!premiseCidr.Contains(broadcast))
                {
                    throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(Broadcast)}={Broadcast}] address is not within the [{nameof(NetworkOptions)}.{nameof(NetworkOptions.NodesSubnet)}={NodesSubnet}] subnet.");
                }
                ...
                {
                    throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(NodesSubnet)}={NodesSubnet}] is not a valid IPv4 subnet.");
                }

                if (!premiseCidr.Contains(nodesSubnetCidr))
                {
                    throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(NodesSubnet)}={NodesSubnet}] is not within [{nameof(NetworkOptions)}.{nameof(PremiseSubnet)}={PremiseSubnet}].");
                }
            }
            ...
            foreach (var subnet in subnets)
            {
                foreach (var subnetTest in subnets)
                {
                    ...
                    {
                        throw new KubeDefinitionException($"[{subnet.Name}={subnet.Cidr}] and [{subnetTest.Name}={subnetTest.Cidr}] overlap.");
                    }
                }
            }
            ...
            {
                throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(IngressSubnet)}={IngressSubnet}] is not a valid subnet.");
            }

            if (!IPAddress.TryParse(IngressGateway, out var ingressGateway) || ingressGateway.AddressFamily != AddressFamily.InterNetwork)
            {
                throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(IPAddress)}={IngressGateway}] is not a valid IPv4 address.");
            }

            if (!ingressSubnet.Contains(ingressGateway))
            {
                throw new KubeDefinitionException($"[{nameof(NetworkOptions)}.{nameof(IPAddress)}={IngressGateway}] is within the [{nameof(IngressSubnet)}={IngressSubnet}].");
            }
        }
    }
}
