...
namespace Neon.WSL
{
    ...
    public sealed class Wsl2Proxy : IDisposable
    {
        ...
        public void ConfigureDNS()
        {
            // $todo(jefflill):
            //
            // We're currently hardcoding this to Google DNS which matches
            // out WSL2 cluster definition.  Eventually, we'll probably want
            // the user to be able to customize this via a neonDESKTOP setting.

            var dnsScript =
@"#!/bin/bash

cat <<EOF > /etc/resolv.conf
# DNS configuration file generated by the neon image tool.
