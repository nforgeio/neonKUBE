...
if [ "${IN_NEON_ANSIBLE_COMMAND}" == "" ] ; then
    ...
    echo "*** ERROR: The [neon_docker_secret] module runs only on the Ansible master.  Consider using: [delegate_to: localhost]" 1>&2
    exit 1
fi

# Invoke the module as a [neon-cli] command.

neon ansible module neon_docker_secret $@
