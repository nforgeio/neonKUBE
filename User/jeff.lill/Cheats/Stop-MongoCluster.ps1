#------------------------------------------------------------------------------
# Stop (deallocate) the virtual machines in a Mongo cluster.
...
Write-Output ""
Write-Output "Stopping ROUTER VMs for: $serviceNamePrefix"
Write-Output "-------------------------------------------------"
...
for($serverId=0; $serverId -lt $routerCount; $serverId++){
    ...
    Write-Output "Stopping: $vmInstanceName"
	Stop-AzureVM -Name $vmInstanceName -ServiceName $routerServiceName -Force
}

Write-Output ""
Write-Output "Stopping DATA VMs for: $serviceNamePrefix"
Write-Output "-------------------------------------------------"
...
for($shardId=0; $shardId -lt $shardCount; $shardId++){
    ...
    for($replicaId=0; $replicaId -lt $replicaCount; $replicaId++){
		...
		Write-Output "Stopping: $vmInstanceName"
		Stop-AzureVM -Name $vmInstanceName -ServiceName $dataServiceName -Force
    }
}
