...
type (
	...
	// cadence Activity to the ActivityID..
	ActivityMap struct {
		sync.Mutex
		activites map[int64]Activity
	}

	// Activity represents a cadence activity workflow execution.
	// It holds a workflow Future and cancellation function
	Activity struct {
		future     workflow.Future     // cadence Future
...
// set as a Activity's cadence workflow.Future
