...
var (
	...
	// a new temporal workflow.Context is created
	contextID int64
)
...
type (
	...
	// running temporal workflow instances (as *WorkflowContext)
	ContextsMap struct {
		sync.Mutex
		contexts map[int64]*Context
	}

	// Context represents a running temporal
	// workflow instance
	Context struct {
		sync.Mutex                              // allows us to safely iterate ID iterator
		workflowName  *string                   // string name of the workflow
		ctx           workflow.Context          // the temporal workflow context
		cancelFunc    workflow.CancelFunc       // temporal workflow context cancel function
...
//
// returns workflow.Context -> a temporal workflow context
...
//
// param value workflow.Context -> a temporal workflow context to be
// set as a WorkflowContext's temporal workflow.Context
...
//
// returns *string -> a temporal workflow function name
...
//
// param value *string -> a temporal workflow function name
...
//
// returns *ChildContextsMap -> a temporal workflow child contexts map
...
//
// param value *ChildContextsMap -> a temporal workflow child contexts map
func (wectx *Context) SetChildContexts(value *ChildContextsMap) {
	wectx.childContexts = value
}

// AddChildContext adds a new temporal context and its corresponding ContextId into
...
//
// param value *proxyactivity.FuturesMap -> a temporal workflow activity futures map
...
// execute workflow functions. This will be the mapped value
//
// returns int64 -> long id of the new temporal WorkflowContext added to the map
