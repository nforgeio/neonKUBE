...
type (
	...
	IWorkflowReply interface {
		...
		SetReplayStatus(status internal.ReplayStatus)
	}
)
...
// state of a workflow
func (reply *WorkflowReply) GetReplayStatus() internal.ReplayStatus {
...
// state of a workflow
func (reply *WorkflowReply) SetReplayStatus(value internal.ReplayStatus) {
	status := value.String()
