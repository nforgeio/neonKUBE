...
$env:GOOS	= "windows"
$env:GOARCH = "amd64"
go build -i -mod=vendor -ldflags="-w -s" -v -o $buildPath\cadence-proxy.win.exe cmd\cadenceproxy\main.go *> "$logPath"
...
$env:GOOS   = "linux"
$env:GOARCH = "amd64"
go build -i -mod=vendor -ldflags="-w -s" -v -o $buildPath\cadence-proxy.linux cmd\cadenceproxy\main.go *> "$logPath"
...
$env:GOOS   = "darwin"
$env:GOARCH = "amd64"
go build -i -mod=vendor -ldflags="-w -s" -v -o $buildPath\cadence-proxy.osx cmd\cadenceproxy\main.go *> "$logPath"
...
neon-build gzip "$buildPath\cadence-proxy.linux"   "$neonCadenceResourceFolder\cadence-proxy.linux.gz"   *> "$logPath"
neon-build gzip "$buildPath\cadence-proxy.osx"     "$neonCadenceResourceFolder\cadence-proxy.osx.gz"     *> "$logPath"
neon-build gzip "$buildPath\cadence-proxy.win.exe" "$neonCadenceResourceFolder\cadence-proxy.win.exe.gz" *> "$logPath"
